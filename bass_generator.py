# --- START OF FILE generator/bass_generator.py (感情ベースパターン選択 + 弱拍強化版) ---
import music21
import logging
...
# --- PATCH: Emotion-driven pattern auto-selection START ---
EMOTION_TO_BUCKET_BASS: dict[str, str] = {
    ...
    "nature_memory_floating_sensation_forgiveness": "calm", # calmに追加

    "supported_light_longing_for_rebirth": "groovy",
    "wavering_heart_gratitude_chosen_strength": "groovy",
    "hope_dawn_light_gentle_guidance": "groovy", # groovyに追加
    ...
    # Haruさんの資料に合わせた他の感情キーワードもここに追加できます
    "default": "groovy" # フォールバック用
}

BUCKET_TO_PATTERN_BASS: dict[tuple[str, str], str] = {
    ("calm", "low"):        "root_only",
    ("calm", "medium_low"): "root_fifth",
    ("calm", "medium"):     "bass_half_time_pop", # calm-mediumは少し動きを出す
    ("calm", "medium_high"):"bass_half_time_pop",
    ("calm", "high"):       "walking", # calm-highは少し動きのあるウォーキング

    ("groovy", "low"):        "bass_syncopated_rnb",
    ("groovy", "medium_low"): "walking",
    ("groovy", "medium"):     "bass_walking_8ths", # groovy-mediumは8分ウォーキング
    ("groovy", "medium_high"):"bass_walking_8ths",
    ("groovy", "high"):       "bass_funk_octave", # groovy-highはファンキーに

    ("energetic", "low"):        "bass_quarter_notes", # energetic-lowは刻みから
    ("energetic", "medium_low"): "bass_pump_8th_octaves",
    ("energetic", "medium"):     "bass_pump_8th_octaves",
    ("energetic", "medium_high"):"bass_funk_octave",
    ("energetic", "high"):       "bass_funk_octave", # energetic-highもファンキーに
    
    # デフォルトバケット用のフォールバック
    ("default", "low"):        "root_only",
    ("default", "medium_low"): "bass_quarter_notes",
    ("default", "medium"):     "walking",
    ("default", "medium_high"):"bass_walking_8ths",
...
def choose_bass_pattern_key(section_musical_intent: dict) -> str:
    ...
    bucket = EMOTION_TO_BUCKET_BASS.get(emotion, "default") # 未知のemotionはdefaultバケットへ
    
    pattern_key = BUCKET_TO_PATTERN_BASS.get((bucket, intensity), "bass_quarter_notes") # マッピング失敗時は基本の四分音符
    ...
    return pattern_key
# --- PATCH: Emotion-driven pattern auto-selection END ---


class BassGenerator:
    # ... (以前の __init__, _get_rhythm_pattern_details, _get_bass_pitch_in_octave, _generate_notes_from_fixed_pattern, _generate_algorithmic_pattern は変更なし) ...
    def __init__(self, rhythm_library: Optional[Dict[str, Dict]] = None, default_instrument: m21instrument.Instrument = m21instrument.AcousticBass(), global_tempo: int = 120, global_time_signature: str = "4/4", global_key_tonic: str = "C", global_key_mode: str = "major", rng_seed: Optional[int] = None):
        ...
        if "basic_chord_tone_quarters" not in self.bass_rhythm_library:
            self.bass_rhythm_library["basic_chord_tone_quarters"] = {
                ...
                "options": {
                    "base_velocity": 85, "strong_beat_velocity_boost": 15, "off_beat_velocity_reduction": 5, "target_octave": 2,
                    "weak_beat_style": "root", "approach_on_4th_beat": True 
                }
            }
        if "bass_quarter_notes" not in self.bass_rhythm_library: self.bass_rhythm_library["bass_quarter_notes"] = {"description": "Default bass line - simple quarter notes on root.", "tags": ["bass", "default", "quarter_notes", "root"], "pattern_type":"fixed_pattern", "pattern": [{"offset": 0.0, "duration": 1.0, "velocity_factor": 0.75, "type": "root"}, {"offset": 1.0, "duration": 1.0, "velocity_factor": 0.7, "type": "root"}, {"offset": 2.0, "duration": 1.0, "velocity_factor": 0.75, "type": "root"}, {"offset": 3.0, "duration": 1.0, "velocity_factor": 0.7, "type": "root"}]}
    ...
    def _get_rhythm_pattern_details(self, rhythm_key: str) -> Dict[str, Any]:
        # rhythm_key が None や空文字の場合のフォールバックを追加
        if not rhythm_key:
            self.logger.warning("BassGenerator: rhythm_key is empty. Using 'basic_chord_tone_quarters'.")
        ...
        if "pattern_type" in details and isinstance(details["pattern_type"], str):
            if "pattern" not in details and details["pattern_type"] != "algorithmic_chord_tone_quarters": # algorithmic_chord_tone_quarters は pattern 不要
                 details["pattern"] = [] 
        elif "pattern" not in details or not isinstance(details["pattern"], list):
            # algorithmic_chord_tone_quarters は pattern を持たないので、この警告は出さないようにする
            if details.get("pattern_type") != "algorithmic_chord_tone_quarters":
                self.logger.warning(f"Bass rhythm key '{rhythm_key}' has no 'pattern_type' and invalid or missing 'pattern'. Using fallback 'basic_chord_tone_quarters'."); return default_rhythm if default_rhythm else {}
        return details
    ...
    def _generate_notes_from_fixed_pattern( self, pattern: List[Dict[str, Any]], m21_cs: harmony.ChordSymbol, base_velocity: int, target_octave: int, block_offset_ignored: float, block_duration: float, current_scale: Optional[music21.scale.ConcreteScale] = None) -> List[Tuple[float, music21.note.Note]]:
        ...
        for p_event in pattern:
            ...
            elif note_type == "octave_root" and root_pitch_obj: chosen_pitch_base = root_note_obj # 修正: root_pitch_obj
    ...
    def _generate_algorithmic_pattern(self, pattern_type: str, m21_cs: harmony.ChordSymbol, options: Dict[str, Any], base_velocity: int, target_octave: int, block_offset_ignored: float, block_duration: float, current_scale: music21.scale.ConcreteScale, next_chord_root: Optional[pitch.Pitch] = None) -> List[Tuple[float, music21.note.Note]]:
        ...
        if pattern_type == "algorithmic_chord_tone_quarters":
            ...
            weak_beat_style = options.get("weak_beat_style", "root"); approach_on_4th_beat = options.get("approach_on_4th_beat", True)
            ...
            for beat_idx in range(num_beats_to_generate):
                ...
                beat_in_measure = beat_idx % beats_per_measure_in_block; chosen_pitch_base: Optional[pitch.Pitch] = None; current_velocity = base_velocity; note_duration_ql = min(1.0, block_duration - current_rel_offset)
                if beat_in_measure == 0 : chosen_pitch_base = root_note_obj; current_velocity = min(127, base_velocity + strong_beat_vel_boost)
                ...
                elif approach_on_4th_beat and next_chord_root and beat_in_measure == (beats_per_measure_in_block - 1) and beat_idx < num_beats_to_generate -1 :
                    possible_approaches = []; approach_lower_chrom = next_chord_root.transpose(-1)
                    ...
                    else: possible_approaches.append(approach_lower_chrom)
                    ...
                    if possible_approaches: chosen_pitch_base = self.rng.choice(possible_approaches); self.logger.debug(f"BassGen: Approaching next chord {next_chord_root.name} with {chosen_pitch_base.name} on beat {beat_idx+1}")
                    else: chosen_pitch_base = root_note_obj
                    current_velocity = max(1, base_velocity - off_beat_vel_reduction)
                else: 
                    current_velocity = max(1, base_velocity - off_beat_vel_reduction)
                    if weak_beat_style == "third_or_fifth": chosen_pitch_base = self.rng.choice([p for p in [m21_cs.third, m21_cs.fifth] if p] or [root_note_obj])
        ...
        elif pattern_type == "algorithmic_root_only": # 他のアルゴリズムも同様に block_offset を加算しない
    ...
    def compose(self, processed_blocks: Sequence[Dict[str, Any]], return_pretty_midi: bool = False) -> Union[stream.Part, Any]:
        ...
        for blk_idx, blk_data in enumerate(processed_blocks):
            # --- PATCH: Emotion-driven pattern auto-selection (Hook) START ---
            # chordmap からの明示的な指定を優先し、なければ感情ベースで選択
            bass_params_from_chordmap = blk_data.get("part_params", {}).get("bass", {})
            
            # 感情ベースでリズムキーを決定 (chordmapにrhythm_keyやstyleがなければ)
            # blk_data には section の musical_intent が含まれている想定
            # (prepare_processed_stream で blk_intent が blk_data にマージされるか、
            #  あるいは section 全体の musical_intent を直接参照する必要がある)
            # ここでは、blk_data に musical_intent があると仮定
            block_musical_intent = blk_data.get("musical_intent", {}) # prepare_processed_stream で設定される想定
            if not block_musical_intent and "section_name" in blk_data: # もしブロックになければセクションから取得試行
                 # このためには、processed_blocksの各要素に、それが属するセクションのmusical_intentもたせる必要がある
                 # 今回は、blk_data["part_params"]["bass"] にセクションの musical_intent がマージされていることを期待
                 pass


            # 優先順位: 1. コードブロック指定 -> 2. セクション指定 -> 3. 感情LUT -> 4. グローバルデフォルト
            ...
            if not rhythm_key_from_params:
                # blk_data にセクション全体の musical_intent が含まれていることを期待
                # (prepare_processed_stream での対応が必要)
                # ここでは、blk_data["part_params"]["bass"] にセクションの musical_intent が
                # translate_keywords_to_params を経由して反映されていると仮定する。
                # そのため、bass_params_from_chordmap に emotion や intensity が含まれているはず。
                # ただし、choose_bass_pattern_key はセクション全体の musical_intent を期待するので、
                # ここでは blk_data (processed_block全体) を渡すのが適切かもしれない。
                # 今回は、bass_params_from_chordmap に必要な情報があると仮定して進める。
                # もしなければ、デフォルトの "basic_chord_tone_quarters" が使われる。
                
                # 実際には、prepare_processed_stream で各ブロックにセクションの musical_intent を含めるか、
                # ここで processed_blocks からセクション情報を再取得する必要がある。
                # 今回は、part_params.bass に emotion/intensity が含まれていることを期待する。
                # もし含まれていなければ、choose_bass_pattern_key はデフォルトを返す。
                # より堅牢なのは、blk_data に section_musical_intent を持たせること。
                # ここでは、blk_data 自体を渡して、choose_bass_pattern_key 側で musical_intent を取得する。
                rhythm_key_from_params = choose_bass_pattern_key(blk_data) # blk_data を渡す
                self.logger.info(f"Bass rhythm key for block {blk_idx} chosen by emotion: {rhythm_key_from_params}")


            if not rhythm_key_from_params: # それでも決まらない場合の最終フォールバック
                rhythm_key_from_params = "basic_chord_tone_quarters"
            # --- PATCH: Emotion-driven pattern auto-selection (Hook) END ---

            if not bass_params_from_chordmap: # パラメータが全くない場合はデフォルトを設定
                bass_params_from_chordmap = {"rhythm_key": rhythm_key_from_params, "velocity": 70, "octave": 2}
                self.logger.debug(f"BassGenerator: No bass params for block {blk_idx+1}. Using emotion-driven or hardcoded defaults.")
            ...
            pattern_details = self._get_rhythm_pattern_details(rhythm_key_from_params) # 感情ベースで選ばれたキーを使用
            ...
            base_vel = bass_params_from_chordmap.get("velocity", pattern_details.get("velocity_base", 70)) # パターン定義のvelも考慮
            target_oct = bass_params_from_chordmap.get("octave", pattern_details.get("target_octave", 2)) # パターン定義のoctも考慮
            ...
            if m21_cs_obj:
                if "pattern_type" in pattern_details and isinstance(pattern_details["pattern_type"], str):
                    ...
                    # chordmapからのパラメータをalgo_optionsに優先的にマージ
                    algo_options.update(bass_params_from_chordmap.get("options", {}))

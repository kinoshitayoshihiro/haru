# --- START OF FILE modular_composer.py (2023-05-24 ボーカル対応版) ---
import music21
import sys
import os
import json
...
try:
    from generator.core_music_utils import get_time_signature_object, sanitize_chord_label
    from generator.piano_generator import PianoGenerator
    from generator.drum_generator import DrumGenerator
    from generator.guitar_generator import GuitarGenerator # GuitarGeneratorをインポート
    from generator.chord_voicer import ChordVoicer
    from generator.melody_generator import MelodyGenerator
    from generator.bass_generator import BassGenerator
    from generator.vocal_generator import VocalGenerator   # ★ VocalGenerator をインポート
...
DEFAULT_CONFIG = {
    ...
    "parts_to_generate": {
        ...
        "chords": True, "melody": False, "vocal": True # ★ vocal を True に (デフォルトで生成)
    },
    "default_part_parameters": {
        ...
        "vocal": { # ★ 新規追加: ボーカル用のデフォルトパラメータ ★
            "instrument": "Vocalist",
            "data_paths": { # デフォルトのデータパス (chordmap.jsonや引数で上書き可能)
                "midivocal_data_path": "data/vocal_note_data_ore.json",
                "lyrics_text_path": "data/kasi_rist.json",
                "lyrics_timeline_path": "data/lyrics_timeline.json" # 現在は直接使われていないが将来用
            },
            "default_insert_breaths_opt": True,
            "default_breath_duration_ql_opt": 0.25,
            "default_humanize_opt": True,
            "default_humanize_template_name": "vocal_ballad_smooth", # humanizer.py/vocal_generator.py内のテンプレートキー
            "default_humanize_time_var": 0.02,
            "default_humanize_dur_perc": 0.04,
            "default_humanize_vel_var": 5,
            "default_humanize_fbm_time": True,
            "default_humanize_fbm_scale": 0.01,
            "default_humanize_fbm_hurst": 0.65
            # "vocal_density_threshold_quiet": 1, # 将来のボーカル密度連携用
            # "vocal_density_threshold_active": 3,
        },
        "chords": {
...
def translate_keywords_to_params(
    ...
    mode_of_block = chord_block_specific_hints.get("mode_of_block", "major").lower() # prepare_processed_stream で設定
    ...
    if instrument_name_key == "piano":
        ...
        if not params.get("piano_rh_rhythm_key") or params.get("piano_rh_rhythm_key") not in piano_patterns_lib:
             fallback_rh = style_to_rhythm_map.get("default_piano_rh_fallback_rhythm", "default_piano_quarters")
             logger.warning(f"Piano RH rhythm for style '{rh_style_kw}' invalid. Fallback: '{fallback_rh}'."); params["piano_rh_rhythm_key"] = fallback_rh
        ...
        if not params.get("piano_lh_rhythm_key") or params.get("piano_lh_rhythm_key") not in piano_patterns_lib:
             fallback_lh = style_to_rhythm_map.get("default_piano_lh_fallback_rhythm", "piano_lh_whole_notes")
             logger.warning(f"Piano LH rhythm for style '{lh_style_kw}' invalid. Fallback: '{fallback_lh}'."); params["piano_lh_rhythm_key"] = fallback_lh
        ...
        if not params.get("drum_style_key") or params.get("drum_style_key") not in drum_patterns_lib:
            logger.warning(f"Drum style key '{params.get('drum_style_key')}' invalid. Fallback 'default_drum_pattern'."); params["drum_style_key"] = "default_drum_pattern"
        ...
        # パラメータ解決 (paramsになければspecific_style_config、それでもなければcfg_guitarのdefault)
        param_keys_guitar = ["guitar_style", "guitar_rhythm_key", "guitar_voicing_style", "guitar_num_strings",
                             "guitar_target_octave", "guitar_velocity", "arpeggio_type", "arpeggio_note_duration_ql",
                             "strum_delay_ql", "mute_note_duration_ql", "mute_interval_ql",
                             "guitar_humanize", "guitar_humanize_style_template", "guitar_humanize_time_var",
                             "guitar_humanize_dur_perc", "guitar_humanize_vel_var", "guitar_humanize_fbm_time",
                             "guitar_humanize_fbm_scale"]
        for p_key in param_keys_guitar:
            if p_key not in params: # chordmapでの直接指定がなければ
                # specific_style_config (emotion/mode map) から取得試行
                # キー名から "guitar_" プレフィックスを除いて specific_style_config を検索
        ...
        if not params.get("guitar_rhythm_key") or params.get("guitar_rhythm_key") not in guitar_rhythm_patterns_lib:
            fallback_guitar_key = cfg_guitar.get("default_rhythm_key", "guitar_default_quarters")
            logger.warning(f"Guitar rhythm key '{params.get('guitar_rhythm_key')}' invalid. Using fallback '{fallback_guitar_key}'.")
            params["guitar_rhythm_key"] = fallback_guitar_key
            
    elif instrument_name_key == "vocal": # ★ ボーカル用のパラメータ解決ロジック ★
        cfg_vocal = DEFAULT_CONFIG["default_part_parameters"]["vocal"]
        # params に cfg_vocal のデフォルト値を設定 (chordmap での上書きがあればそれが優先される)
        # data_paths はここでは解決しない (run_compositionで直接扱う)
        vocal_param_keys = ["insert_breaths_opt", "breath_duration_ql_opt", "humanize_opt",
                            "humanize_template_name", "humanize_time_var", "humanize_dur_perc",
                            "humanize_vel_var", "humanize_fbm_time", "humanize_fbm_scale", "humanize_fbm_hurst"]
        for p_key_vocal in vocal_param_keys:
            if p_key_vocal not in params: # chordmapでの直接指定がなければ
                params[p_key_vocal] = cfg_vocal.get(f"default_{p_key_vocal}")
        
        # 将来的なボーカル特徴量に基づくパラメータ調整の例
        # vocal_features = chord_block_specific_hints.get("vocal_features", {}) # prepare_processed_stream で設定想定
        # if vocal_features:
        #     density = vocal_features.get("density_score", 0)
        #     if density <= params.get("vocal_density_threshold_quiet", 1) : # 例: 静かなら
        #         params["humanize_vel_var"] = max(1, params.get("humanize_vel_var", 5) - 2)
        #     elif density >= params.get("vocal_density_threshold_active", 3): # 例: 忙しいなら
        #         params["humanize_time_var"] = max(0.005, params.get("humanize_time_var", 0.02) - 0.005)


    elif instrument_name_key == "melody": # (変更なし)
        ...
        if not params.get("rhythm_key") or params.get("rhythm_key") not in melody_rhythms_lib:
            logger.warning(f"Melody rhythm key '{params.get('rhythm_key')}' invalid. Fallback."); params["rhythm_key"] = "default_melody_rhythm"
    ...
    elif instrument_name_key == "bass": # (変更なし)
        ...
        if not params.get("rhythm_key") or params.get("rhythm_key") not in bass_lines_lib:
            logger.warning(f"Bass rhythm key '{params.get('rhythm_key')}' invalid. Fallback."); params["rhythm_key"] = "bass_quarter_notes"

    # ブロック固有のヒントで最終上書き (part_settings は既に適用済み)
    # instrument_name_key に一致するキーが chord_block_specific_hints にあれば、その内容で params を更新
    ...
    if instrument_name_key == "drums" and "drum_fill" in chord_block_specific_hints: # これはトップレベルのヒント
...
def prepare_processed_stream(chordmap_data: Dict, main_config: Dict, rhythm_lib_all: Dict) -> List[Dict]:
    ...
    for sec_name, sec_info in sorted_sections:
        ...
        sec_t = sec_info.get("tonic", g_key_t)
        sec_m = sec_info.get("mode", g_key_m) # ★ セクションのモードを取得
        ...
        for c_idx, c_def in enumerate(chord_prog):
            ...
            # blk_hints_for_translate には、セクションレベルの part_settings と、ブロック固有のヒント、モード情報を含める
            blk_hints_for_translate = {"part_settings": sec_part_settings_for_all_instruments.copy()}
            current_block_mode = c_def.get("mode", sec_m) # コードブロック固有のモード指定があれば優先
            blk_hints_for_translate["mode_of_block"] = current_block_mode # ★ ブロックのモード情報を渡す
            ...
            blk_data = {
                ...
                "section_name": sec_name, "tonic_of_section": sec_t, "mode": current_block_mode, # ★ ブロックの実際のモードを格納
            ...
            for p_key_name, generate_flag in main_config.get("parts_to_generate", {}).items():
                if generate_flag:
                    ...
                    blk_data["part_params"][p_key_name] = translate_keywords_to_params(
                        blk_intent, 
                        blk_hints_for_translate, # ここに mode_of_block が含まれる
...
def run_composition(cli_args: argparse.Namespace, main_cfg: Dict, chordmap: Dict, rhythm_lib_all: Dict):
    ...
    if main_cfg["parts_to_generate"].get("piano"):
        gens["piano"] = PianoGenerator(rhythm_library=cast(Dict[str,Dict], rhythm_lib_all.get("piano_patterns", {})), chord_voicer_instance=cv_inst, global_tempo=main_cfg["global_tempo"], global_time_signature=main_cfg["global_time_signature"])
    if main_cfg["parts_to_generate"].get("drums"):
        gens["drums"] = DrumGenerator(drum_pattern_library=cast(Dict[str,Dict[str,Any]], rhythm_lib_all.get("drum_patterns", {})), global_tempo=main_cfg["global_tempo"], global_time_signature=main_cfg["global_time_signature"])
    if main_cfg["parts_to_generate"].get("guitar"):
        guitar_cfg = main_cfg["default_part_parameters"].get("guitar", {})
        gens["guitar"] = GuitarGenerator(
            rhythm_library=cast(Dict[str,Dict], rhythm_lib_all.get(guitar_cfg.get("default_rhythm_category","guitar_patterns"), {})),
            default_instrument=m21instrument.fromString(guitar_cfg.get("instrument", "AcousticGuitar")),
            global_tempo=main_cfg["global_tempo"], global_time_signature=main_cfg["global_time_signature"]
        )
    if main_cfg["parts_to_generate"].get("vocal"): # ★ VocalGeneratorのインスタンス化 ★
        vocal_cfg = main_cfg["default_part_parameters"].get("vocal", {})
        vocal_data_paths = vocal_cfg.get("data_paths", {})
        
        # データファイルのパスを解決 (chordmap -> DEFAULT_CONFIG)
        midivocal_path_str = chordmap.get("global_settings", {}).get("vocal_mididata_path", vocal_data_paths.get("midivocal_data_path"))
        lyrics_path_str = chordmap.get("global_settings", {}).get("vocal_lyrics_path", vocal_data_paths.get("lyrics_text_path"))
        timeline_path_str = chordmap.get("global_settings", {}).get("vocal_timeline_path", vocal_data_paths.get("lyrics_timeline_path"))
        ...
        lyrics_timeline_data = load_json_file(Path(timeline_path_str), "Lyrics Timeline Data") if timeline_path_str else None # 現在未使用だがロードは試みる

        if midivocal_data and kasi_rist_data: # 最低限これらのデータが必要
            gens["vocal"] = VocalGenerator(
                default_instrument=m21instrument.fromString(vocal_cfg.get("instrument", "Vocalist")),
                global_tempo=main_cfg["global_tempo"],
                global_time_signature=main_cfg["global_time_signature"]
            )
        else:
            logger.error("Vocal part generation skipped: Missing essential vocal data (MIDI data or lyrics).")
            main_cfg["parts_to_generate"]["vocal"] = False # 生成フラグをオフに
    ...
    for p_n, p_g_inst in gens.items():
        if p_g_inst and main_cfg["parts_to_generate"].get(p_n): # ジェネレータが存在し、生成フラグがTrue
            logger.info(f"Generating {p_n} part...")
            try:
                if p_n == "vocal": # VocalGeneratorは特別な引数を取る
                    vocal_params_from_config = main_cfg["default_part_parameters"].get("vocal", {})
                    # translate_keywords_to_params で解決されたパラメータを使うべきだが、
                    # Vocalはブロックごとのパラメータというより全体的な設定が主なので、
                    # ここでは DEFAULT_CONFIG の値を直接参照する形で簡略化。
                    # 必要なら、proc_blocks からボーカル用パラメータを集約する。
                    part_obj = p_g_inst.compose(
                        midivocal_data=cast(List[Dict], midivocal_data), # run_compositionのスコープでロード済み
                        kasi_rist_data=cast(Dict[str, List[str]], kasi_rist_data),
                        lyrics_timeline_data=cast(List[Dict], lyrics_timeline_data), # VocalGenerator内で現在未使用
                        processed_chord_stream=proc_blocks, # セクション特定用
                        insert_breaths_opt=vocal_params_from_config.get("default_insert_breaths_opt", True),
                        breath_duration_ql_opt=vocal_params_from_config.get("default_breath_duration_ql_opt", 0.25),
                        humanize_opt=vocal_params_from_config.get("default_humanize_opt", True),
                        humanize_template_name=vocal_params_from_config.get("default_humanize_template_name"),
                        humanize_custom_params={
                            k.replace("default_humanize_", ""):v 
                            for k,v in vocal_params_from_config.items() 
                            if k.startswith("default_humanize_") and not k.endswith("_template_name") and not k.endswith("_opt")
                        }
                    )
                else:
...
def main_cli():
    ...
    # parser.add_argument("--vocal-timeline-path", type=Path, help="Path to lyrics timeline JSON.") # 現在未使用だが将来用
    ...
    # if args.vocal_timeline_path: effective_config["default_part_parameters"]["vocal"]["data_paths"]["lyrics_timeline_path"] = str(args.vocal_timeline_path)
    ...
    if args.tempo is not None: effective_config["global_tempo"] = args.tempo; logger.info(f"Tempo overridden by CLI: {args.tempo}")

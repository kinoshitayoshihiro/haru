...
from typing import Optional, Dict, Any, List # Dict をインポート
import re
...
def sanitize_chord_label(label: Optional[str]) -> Optional[str]:
    ...
    # 0a. ワードベースの品質変換 (o3さん提案 - re.subの置換を関数に変更)
    word_map_patterns = {
        r'(?i)\b([A-Ga-g][#b\-]*)\s+minor\b': lambda m: m.group(1) + 'm',
        r'(?i)\b([A-Ga-g][#b\-]*)\s+major\b': lambda m: m.group(1) + 'maj',
    ...
    for pat, repl_func in word_map_patterns.items():
        try:
            sanitized = re.sub(pat, repl_func, sanitized)
        except Exception as e_re_sub:
            logger.error(f"CoreUtils (sanitize): Error in re.sub for pattern '{pat}' with label '{sanitized}': {e_re_sub}")
            pass
    
    # 0b. ルート音の先頭文字を大文字化、それ以外を小文字化
    ...
    if open_paren_count > close_paren_count and open_paren_count == 1 :
    ...
    # 3. altコード展開
    sanitized = re.sub(r'([A-Ga-g][#\-]?)(?:7)?alt(?:ered)?', r'\g<1>7#9b13', sanitized, flags=re.IGNORECASE)
    ...
    if match_paren:
        ...
        if sanitized != original_label: logger.debug(f"CoreUtils (sanitize): Parentheses flattened: '{original_label}' -> '{sanitized}'")
    ...
    qual_map = {
        ...
        r'(?i)major(?![\d#b])': '', r'(?i)maj(?![\d#b])': '', r'(?i)M(?!aj|\d|[#b])': '',
    }
    ...
    if sanitized.lower().endswith("add") and not (len(sanitized) > 3 and sanitized.lower()[-4:-3].isdigit()): 
        logger.debug(f"CoreUtils (sanitize): Removing trailing 'add' from '{sanitized}' (orig: '{original_label}')")
    ...
    try:
        ...
        if not cs_test.pitches: 
            ...
            return None
        if not cs_test.root(): # ルート音がない場合も無効
            logger.warning(f"CoreUtils (sanitize): Final form '{sanitized}' (from '{original_label}') parsed but has NO ROOT. Fallback to None (Rest).")
            return None
    ...
    if not re.match(r'^[A-G]', sanitized.split('/')[0]): # スラッシュコードのベース音の前もチェック
        logger.warning(f"CoreUtils (sanitize): Final form '{sanitized}' (main part: {sanitized.split('/')[0]}) does not start with valid note name. Fallback None (Rest).")
        return None
...
def get_music21_chord_object(chord_label_str: Optional[str], current_key: Optional[scale.ConcreteScale] = None) -> Optional[harmony.ChordSymbol]:
    ...
    try:
        ...
        root_match = _ROOT_RE_STRICT.match(sanitized_label) 
        if not root_match: root_match = _ROOT_RE_SIMPLE.match(sanitized_label)
        if root_match:
...
if __name__ == '__main__':
    ...
    final_expected_outcomes_masterpiece_final_polish = {
        ...
        "Am7(add11)": "Am7add11", "Am7(add11": "Am7add11", "Dsus": "Dsus4", "Gsus4": "Gsus4", 
        "Asus2": "Asus2", "Csus44": "Csus4", "Bbmaj9(#11)": "B-maj7#11add9", "F#7": "F#7", 
        "Calt": "C7#9b13", "silence": None, "Cminor7": "Cm7", "Gdominant7": "G7", "Bb": "B-", 
        "Ebm": "E-m", "F#": "F#", "Dbmaj7": "D-maj7", "G7SUS": "G7sus4", "G7sus": "G7sus4", 
        "Gsus": "Gsus4", "d minor": "Dm", "e dim": "Edim", "C major7": "Cmaj7", "G AUG": "Gaug", 
        "C major": "C", "bad(input": None, "": None, "N.C.": None, "  Db  ": "D-", "GM7(": "GM7", 
        "G diminished": "Gdim", "C/Bb": "C/B-", "CbbM7": "C--M7", "C##M7": "C##M7", 
        "C(omit3)": "Comit3", "Fmaj7(add9": "Fmaj7add9", "Am7addadd11": "Am7add11", "Gadd": "Gadd", 
        "Fmaj7add": "Fmaj7", "Dm7addadd13": "Dm7add13", "Cadd": "Cadd", 
        "Fmaj7addaddadd13": "Fmaj7add13", "B-maj9#11add": "B-maj7#11add9", "Gsus2/D": "Gsus2/D", 
        "Cadd9": "Cadd9", "Am(add9)": "Amadd9", "Csus4/G": "Csus4/G", "Am7add11": "Am7add11", 
        "G13": "G13", "Bo7": "Bdim7", "E7b9": "E7b9", "Fmaj7add9": "Fmaj7add9", "C/E": "C/E", 
        "Gm7b5": "Gm7b5", "C7#9b13": "C7#9b13", "Dm7add13": "Dm7add13", "Em7b5": "Em7b5", 
        "A7b9": "A7b9", "C13": "C13", "Bbmaj9(#11)": "B-maj7#11add9", "C/Bb": "C/B-", 
        "D7sus4": "D7sus4", "Fmaj7add9add13": "Fmaj7add13", "Ebmaj7(#11)": "E-maj7#11", 
        "Bbmaj7/D": "B-maj7/D",
    }
    
    other_tests_masterpiece_final_polish = [ "Rest", "Dsus2", "Dsus4", "Csu" ] 
    ...
    for label_orig in all_labels_to_test_masterpiece_final_polish:
        ...
        if cs_obj_mfp:
            ...
            pitches_str = ", ".join(sorted(list(set(p.name for p in cs_obj_mfp.pitches)))) if cs_obj_mfp.pitches else "No Pitches"
            print(f"  └─> music21 obj: '{fig_disp:<25}' (Unique Pitch Classes: {pitches_str})")
            s_parses_mfp += 1
        elif sanitized_res is None : 
